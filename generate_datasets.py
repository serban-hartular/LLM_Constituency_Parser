import pickle
from main import TokenSpan, TrainingDatum
...
if __name__ == "__main__":
    with open('./datasets/raw_data-dev.p', 'rb') as handle:
        raw_training_dev = pickle.load(handle)

    phrase_head_qa = [extract_phrase_head(td) for td in raw_training_dev]
    phrase_head_qa = [d for d in phrase_head_qa if d]
    ds = Dataset.from_list(phrase_head_qa)
    ds_dict = ds.train_test_split(0.25)
    ds_dict.save_to_disk('./datasets/dsdict_qa_head-dev-1')
